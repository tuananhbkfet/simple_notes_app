...
export function NotesApp({ initialGroup = undefined }: NotesAppProps) {
  ...
  // State cho việc tạo ảnh
  const [imagePrompt, setImagePrompt] = useState("");
  const [generatedImageUrl, setGeneratedImageUrl] = useState<string | null>(null);
  const [isGenerating, setIsGenerating] = useState(false);
  const generateImageAction = useAction(api.imageGeneration.generateImage);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      // 1. Generate image, get base64 string
      const base64string = await generateImageAction({ prompt: imagePrompt });
      ...
      const imageFile = new File([blob], "generated.png", { type: "image/png" });
      ...
      // 5. Get the public URL for the stored file
      const publicUrl = await getUrlFromStorageId({ storageId });

      if (!publicUrl) {
          throw new Error("Could not get image URL after upload.");
      }

      // 6. Set the public URL to be displayed
      setGeneratedImageUrl(publicUrl);
      toast.success("Đã tạo và tải ảnh lên thành công!");
  ...
  const handleAddToNote = async () => {
    ...
    setGeneratedImageUrl(null);
    setImagePrompt("");
    toast.success("Đã thêm ảnh vào ô soạn thảo");
  };
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="Hoặc... tạo ảnh từ văn bản (dùng Gemini 2.5 Flash)"
              ...
              <button
                ...
                className="px-4 py-2 bg-purple-500 text-white rounded-lg font-medium hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            {generatedImageUrl && (
              <div className="flex flex-col items-center gap-2">
                <img src={generatedImageUrl} alt="Generated by AI" className="rounded-lg max-w-full h-auto" />
                <button
                  type="button"
              ...
              {editingId === note._id ? (
                ...
                <div>
                  <div className="flex items-start mb-4">
                    ...
                    <div className="prose prose-sm max-w-none text-gray-800 whitespace-pre-wrap leading-relaxed text-base">
